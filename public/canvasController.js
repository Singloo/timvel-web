 ...
 * -----
 * Last Modified: Monday May 6th 2019 12:10:24 pm
...
var Starfield = /** @class */ (function() {
  function Starfield() {
    ...
    //  document.getElementsByTagName('canvas')[0] || document.createElement('canvas');
    this.dx = 0;
    this.dy = 0;
  ...
  Starfield.prototype.initStarCount = function() {
    var _this = this;
    (_a = this.stars).push.apply(
      _a,
      Array.from({ length: COUNT_MAX }).map(function() {
        return _this.starAttributes;
      }),
    );
    var _a;
  };
  ...
  Starfield.prototype.initGui = function() {
    ...
    gui
      .add(this.settings, 'count', 100, COUNT_MAX, 10)
      .listen()
      .onChange(function() {
  ...
  Starfield.prototype.draw = function() {
    ...
    if (this.settings.demo) {
      ...
      this.settings.cursor.x = ~~(
        this.fW2 +
        this.fW2 *
          Math.sin(this.cursorPosition) *
          Math.tan(Math.sin(this.cursorPosition * 1.5))
      );
      this.settings.cursor.y = ~~(
        this.fH2 +
        this.fH2 *
          Math.sin(this.cursorPosition * 2) *
          Math.tan(Math.cos(this.cursorPosition))
      );
    }
    for (var i = 0; i < this.currentCount; i += 1) {
      var star = this.stars[i];
      var x =
        (baseWidth + star.x * this.width + Math.sin(star.angle) * position) %
        this.width;
      var y =
        (baseHeight + star.y * this.height + Math.cos(star.angle) * position) %
        this.height;
      ...
      if (this.settings.cursor.impact > 0) {
        ...
        if (this.clickStart) {
          ...
          if (this.settings.cursor.clickDuration < elapsed) {
            ...
            cursorRadius +=
              (elapsed / this.settings.cursor.clickDuration) *
              this.settings.cursor.clickSize;
          }
        }
        var distance = Math.sqrt(
          Math.pow(x - this.settings.cursor.x, 2) +
            Math.pow(y - this.settings.cursor.y, 2),
        );
        if (
          cursorRadius < distance &&
          distance < cursorRadius + this.settings.cursor.size.range
        ) {
          impact =
            this.settings.cursor.impact *
            Math.sin(
              ((distance - cursorRadius) * Math.PI) /
                this.settings.cursor.size.range,
            );
        }
      }
      this.ctx.moveTo(x, y);
